...
class VLGHead(nn.Module):
    ...
    def forward(self, inputs, force_output_pred_masks=False):
        ...
        if text_feats.dim() == 2:
            # [N, C] -> [B, N, C]
            text_feats = text_feats.unsqueeze(0).repeat(B, 1, 1).float()
        elif text_feats.dim() == 3:
            if text_feats.size(0) != B:
                raise RuntimeError(f"text_feats batch {text_feats.size(0)} != image batch {B}")
