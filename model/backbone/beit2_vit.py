...
class BEiT2VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 out_indices=[3, 5, 7, 11],pretrained=None,**kwargs):
        super().__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        Args:
            pretrained (str or dict, optional): Path to ckpt or a state_dict-like object.
        """
        ...
        if isinstance(pretrained, (dict, OrderedDict)):
            raw = pretrained
        else:
            logger.info(f'Loading BEiT2 backbone ckpt: {pretrained}')
            raw = torch.load(pretrained, map_location='cpu')

        sd = None
        for k in ['state_dict', 'model', 'module', 'net', 'ema_state_dict']:
            if isinstance(raw, dict) and k in raw and isinstance(raw[k], (dict, OrderedDict)):
                sd = raw[k]; break
        if sd is None:
            sd = raw if isinstance(raw, (dict, OrderedDict)) else {}

        drop_prefixes = ('state_dict.', 'module.', 'model.', 'encoder.', 'visual.', 'backbone.')
        cleaned = OrderedDict()
        for k, v in sd.items():
            nk = k
        ...
        ret = self.load_state_dict(loadable, strict=False)

        missing = [k for k in ret.missing_keys if not k.startswith(('fpn1', 'fpn2', 'fpn3', 'fpn4'))]
        unexpected = ret.unexpected_keys 
        logger.info(
            f'BEiT2 ckpt loaded: matched={len(loadable)} | '
            f'coverage_ckpt={len(set(loadable)&set(cleaned))/max(1,len(cleaned)):.3f} | '
            f'coverage_model={len(loadable)/max(1,len(model_sd)):.3f} | '
            f'missing={len(missing)} | unexpected={len(unexpected)} | '
            f'shape_mismatch={len(shape_mismatch)}'
        )
        if len(missing) > 0:
            logger.debug(f'[missing(sample)] {missing[:20]}')
        if len(unexpected) > 0:
            logger.debug(f'[unexpected(sample)] {unexpected[:20]}')
        if len(shape_mismatch) > 0:
            logger.debug(f'[shape_mismatch(sample)] {shape_mismatch[:10]}')
    ...
    def forward_features(self, x):
        ...
        B2, N, C2 = x.shape
        assert B2 == B and C2 == self.embed_dim

        cls_tokens = self.cls_token.expand(B, -1, -1)  # [B,1,C]
        x = torch.cat((cls_tokens, x), dim=1)          # [B, 1+N, C]
        if self.pos_embed is not None:
            pos = _interp_abs_pos_embed(self.pos_embed, Hp, Wp)  # [1, 1+N, C]
            x = x + pos
        x = self.pos_drop(x)
