...
from third_party.zegclip.models.backbones.utils import LayerNorm
...
class TransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = MultiheadAttention(
            ...
            bias=qkv_bias)
        if lora:
            self.lora = LoRA(embed_dims, lora_r, lora_scaling, lora_dropout, lora_targets)
        else:
            self.lora = None
    ...
    def forward(self, x, return_qkv=False):
        ...
        if return_qkv:
            if self.lora is not None:
                q, k, v = self.lora.forward_qkv(self.attn, self.norm1(x), identity=x)
                if self.first_iter:
                    oq, ok, ov = self.forward_qkv(x)
                    assert torch.allclose(q, oq), f'{q}\n{oq}'
                    assert torch.allclose(k, ok), f'{k}\n{ok}'
                    assert torch.allclose(v, ov), f'{v}\n{ov}'
            else:
        ...
        if self.lora is not None:
            if self.first_iter:
                ox = self.attn(self.norm1(x), identity=x)
            x = self.lora(self.attn, self.norm1(x), identity=x)
            if self.first_iter:
                assert torch.allclose(x, ox, atol=1e-05), f'{x}\n{ox}'
        else:
...
class MaskClipVisionTransformer(BaseModule):
    """Vision Transformer.
